...
def crawl(seed, count_seed):
    ...
    while((depth < maxDepth) or (len(queue) == 0)):
        ...
        if depth == 1:
            get_html = session.get(currentUrl).content
...
def RetrievePhrase():
    ...
    for element in phrase_arr:
        if (not indexed_words_dic.__contains__(element)):
            return NULL
    ...
    for elem in phrase_arr:
        word_pages_lists.append(indexed_words_dic[elem])
